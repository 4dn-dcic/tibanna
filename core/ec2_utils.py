...
def launch_instance (par, jobid, shutdown_min ):
    ...
    command="./create_run_workflow.sh {jobid} {dir} {shutdown_min}".format(jobid=jobid, dir=par['userdata_dir'], shutdown_min = shutdown_min)
    run_command_out_check (command)
    ...
    if par['storage_iops']:    ## io1 type, specify iops
        launch_command += " --block-device-mappings DeviceName=/dev/sdb, Ebs=\"{{VolumeSize={EBS_SIZE}, VolumeType={EBS_TYPE}, Iops={EBS_IOPS}, DeleteOnTermination=true}}\"".format(EBS_SIZE=par['storage_size'], EBS_TYPE=par['storage_type'], EBS_IOPS=par['storage_iops'])
    else: ## gp type or other type? do not specify iops
        launch_command += " --block-device-mappings DeviceName=/dev/sdb, Ebs=\"{{VolumeSize={EBS_SIZE}, VolumeType={EBS_TYPE}, DeleteOnTermination=true}}\"".format(EBS_SIZE=par['storage_size'], EBS_TYPE=par['storage_type'])
...
if __name__ == "__main__":
    ...
    parser.add_argument("-sT", "--storage_type", help="EBS storage type (available values: gp2, io1, st1, sc1, standard (default: io1)")
    ...
    parser.add_argument("-m", "--shutdown_min", help="Number of minutes before shutdown after the jobs are finished. (default now)")
    ...
    CONFIG_KEYS=["reference_S3_bucket", "output_S3_bucket", "s3_access_arn", "keyname", "worker_ami_id", "default_instance_type", "default_ebs_size", "default_ebs_type", "ebs_iops", "userdata_dir", "json_dir", "json_bucket", "cwl_url", "job_list_file"]
