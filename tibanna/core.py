...
class Deploy(object):
    ...
    def deploy_packaged_lambdas(self, name, suffix=None, dev=False, usergroup=None):
        if name == 'pony_only':
    ...
    def deploy_core(self, name, tests=False, suffix=None, usergroup=None, package='tibanna'):
        ...
        for name in names:
            ...
            with self.chdir(package + "/lambdas/%s" % (name)):
    ...
    def deploy_tibanna(self, suffix=None, sfn_type='pony', usergroup=None, tests=False,
        ...
        step_function_name = self.create_stepfunction(suffix, sfn_type, usergroup=usergroup)
        if setenv:
        ...
        if sfn_type == 'pony':
            self.deploy_core('pony_only', tests=tests, suffix=suffix, usergroup=usergroup, package='tibanna_4dn')
            self.deploy_core('unicorn', tests=tests, suffix=suffix, usergroup=usergroup)
        else:
            self.deploy_core('unicorn', tests=tests, suffix=suffix, usergroup=usergroup)
    ...
    def deploy_unicorn(self, suffix=None, no_setup=False, buckets='',
        ...
        self.deploy_tibanna(suffix=suffix, sfn_type='unicorn',
