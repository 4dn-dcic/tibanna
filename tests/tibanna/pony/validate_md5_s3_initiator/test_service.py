from tibanna_4dn.lambdas.validate_md5_s3_initiator import handler as validate_md5_s3_initiator
from tibanna_4dn.lambdas.validate_md5_s3_initiator import make_input, get_status_for_extra_file
# from tibanna.validate_md5_s3_initiator.import STEP_FUNCTION_ARN
import pytest
...
def test_s3_trigger_e2e(s3_trigger_event_data):
    ...
    with mock.patch('tibanna_4dn.lambdas.validate_md5_s3_initiator.get_status') as uploading:
...
def test_s3_trigger_extra_to_be_uploaded_by_workflow(s3_trigger_event_data_pf_extra_status):
    ...
    with mock.patch('tibanna_4dn.lambdas.validate_md5_s3_initiator.get_status') as uploading:
        uploading.return_value = 'released'
        with mock.patch(
            'tibanna_4dn.lambdas.validate_md5_s3_initiator.get_status_for_extra_file'
        ) as tobeuploadedbywf:
