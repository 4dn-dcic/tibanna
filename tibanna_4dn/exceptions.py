import traceback
from tibanna.exceptions import Exceptions as DefaultExceptions


class Exceptions(DefaultExceptions):
    ...
    def exception_coordinator(self, lambda_name, metadata_only_func):
        '''
        ...
        def decorator(function):
            ...
            ignored_exceptions = [self.EC2StartingException, self.StillRunningException,
                                  self.TibannaStartException, self.FdnConnectionException,
                                  self.DependencyStillRunningException, self.EC2InstanceLimitWaitException]
            ...
            def wrapper(event, context):
                ...
                try:
                    ...
                    if type(e) in ignored_exceptions:
                        ...
                        raise e
                    # else
                    if e.__class__ == self.AWSEMJobErrorException:
                        error_msg = 'Error on step: %s: %s' % (lambda_name, str(e))
                    elif e.__class__ == self.EC2UnintendedTerminationException:
                        error_msg = 'EC2 unintended termination error on step: %s: %s' % (lambda_name, str(e))
                    elif e.__class__ == self.EC2IdleException:
